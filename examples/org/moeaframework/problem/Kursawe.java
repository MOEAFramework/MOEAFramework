...
/**
 * The Kursawe test problem. Features a disconnected Pareto set and Pareto
 * front. According to a personal correspondence between Van Veldhuizen and
 * Marco Laumanns, a misprint exists in Kursawe's paper. This implementation
...
public class Kursawe extends AbstractProblem {
	 ...
	 */
	public Kursawe(int n, double lowerBound, double upperBound) {
		super(n, 2);
		this.n = n;
	...
	public void evaluate(Solution solution) {
		...
		for (int i = 0; i < n - 1; i++) {
			f1 += -10.0
					* Math.exp(-0.2
							* Math.sqrt(Math.pow(x[i], 2.0)
									+ Math.pow(x[i + 1], 2.0)));
		}

		double f2 = 0.0;
		for (int i = 0; i < n; i++) {
			f2 += Math.pow(Math.abs(x[i]), 0.8) + 5.0
					* Math.sin(Math.pow(x[i], 3.0));
		}
	...
	public Solution newSolution() {
		Solution solution = new Solution(n, 2);
