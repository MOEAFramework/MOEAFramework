...
Combining the island-model and master-slave parallelization strategies, the hybrid parallelization approach inherits the benefits of both strategies.  It gains speedup from the master-slave strategy by distributing function evaluations across many cores, and the benefit of operating multiple, concurrent algorithms in the island-model strategy.  Using the parallel computing library JPPF, from \sectref{sect:masterSlave}, we can enable distributed computing with the island-model approach.  To enable the hybrid strategy, you can wrap the problem created on lines 25-26 to enable evaluating on multiple local cores:
...
or on multiple computers across a network using JPPF or another library:
...
\section{Conclusion}
This chapter provided an introduction to parallel computing with the MOEA Framework.  We explored the master-slave, island-model, and hybrid parallelization strategy.  Using parallelization, we can help decrease computing time by distributing the workload across multiple computers and/or improve convergence properties by running multiple concurrent algorithms.  