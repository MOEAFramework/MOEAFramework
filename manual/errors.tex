...
\begin{indented}
  ...
  to global.properties and set \plaintext{<folder>} to some temporary folder where the PISA communication files will be stored.  Then configure your anti-virus software to ignore the contents of \plaintext{<folder>}.
\end{indented}
...
\begin{indented}
  Thrown by \java{ResultFileReader} if either of the following two cases occurs: 1) the binary variable length differs from that specified in the problem definition; and 2) the string encoding in the file contains invalid characters.  In either case, the binary variable stored in the result file could not be read.
\end{indented}
...
instantiated: java.lang.NoClassDefFoundError: \plaintext{<class>}}
\begin{indented}
