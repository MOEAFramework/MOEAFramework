...
It is not vitally important to understand all of the code.  Much of the code is for loading the data file discussed in the previous section.  The key sections of the code are the \java{evaluate} method starting on line 108 and the \java{newSolution} method on line 208.  Starting with the \java{newSolution} method, notice how line 209 create a solution using the three-argument constructor, \java{ew Solution(1, nsacks, nsacks)}.  The three argument constructor is used to define constraints.  In this example, we are defining a problem with \java{1} decision variable, \java{nsacks} objectives, and \java{nsacks} constraints --- one objective and one constraint for each knapsack.  Then on line 210 we set the one decision variable to be a bit string (binary encoding) with \java{nitems} bits.
