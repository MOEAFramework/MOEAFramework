...
public class ContextFreeGrammar {
	...
	/**
	 * Returns the minimum height of any resulting derivation from the given
	 * rule.  A value of {@code Integer.MAX_VALUE} indicates the rule is
	 * infinitely recursive and will never result in a finite size derivation.
	 * 
	 * @param index the index of the rule
	 * @return the minimum height of any resulting derivation from the given
	 *         rule
	 */
	public int getMinimumHeight(int index) {
		return getMinimumHeight(get(index), new Stack<Symbol>());
	}
	 ...
	 * 
	 * @param rule the rule
	 * @param visitedRules the collection of visited rules used to detect
	 *        recursive derivations
	 * @return the minimum height of any resulting derivation from the given
	 *         rule
	 */
	protected int getMinimumHeight(Rule rule, Stack<Symbol> visitedRules) {
		if (visitedRules.contains(rule.getSymbol())) {
			return Integer.MAX_VALUE;
		} else {
			int result = Integer.MAX_VALUE;
			visitedRules.push(rule.getSymbol());
			
			for (int i = 0; i < rule.size(); i++) {
				Production production = rule.get(i);
				int height = 0;
				
				for (int j = 0; j < production.size(); j++) {
					Symbol symbol = production.get(j);
					
					if (!symbol.isTerminal()) {
						height = Math.max(height, getMinimumHeight(get(symbol),
								visitedRules));
					}
				}
