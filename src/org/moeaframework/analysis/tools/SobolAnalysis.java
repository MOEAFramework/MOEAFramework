...
public class SobolAnalysis extends CommandLineUtility {
	...
	public void run(CommandLine commandLine) throws Exception {
		...
		double[] responses = load(input, index);
	 ...
	 * generated by {@link Saltelli}.
	 * 
	 * @param file the model output file
	 * @param index the output column index
	 * @throws IOException if an I/O error occurred
	 */
	private double[] load(File file, int index) throws IOException {
		try (MatrixReader reader = new MatrixReader(file)) {
			List<Double> values = new ArrayList<Double>();
			
			while (reader.hasNext()) {
				values.add(reader.next()[index]);
			}
						
			return values.stream().mapToDouble(x -> x.doubleValue()).toArray();
		}
	}
