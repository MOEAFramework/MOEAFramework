...
public class ZCAT8Test {
	...
	public void testGenerate() {
		TestUtils.assertGeneratedSolutionsAreNondominated(new ZCAT8(3), 1000);
	}
	
	@Test
	public void testProvider() {
		Assert.assertNotNull(ProblemFactory.getInstance().getProblem("ZCAT8_2"));
		Assert.assertNotNull(ProblemFactory.getInstance().getReferenceSet("ZCAT8_2"));
		Assert.assertEquals(2, ProblemFactory.getInstance().getProblem("ZCAT8_2").getNumberOfObjectives());
		
		Assert.assertNotNull(ProblemFactory.getInstance().getProblem("ZCAT8_3"));
		Assert.assertNull(ProblemFactory.getInstance().getReferenceSet("ZCAT8_3"));
		Assert.assertEquals(3, ProblemFactory.getInstance().getProblem("ZCAT8_3").getNumberOfObjectives());
	}
