...
public class ZCAT3Test {
	...
	public void testGenerate() {
		TestUtils.assertGeneratedSolutionsAreNondominated(new ZCAT3(3), 1000);
	}
	
	@Test
	public void testProvider() {
		Assert.assertNotNull(ProblemFactory.getInstance().getProblem("ZCAT3_2"));
		Assert.assertNotNull(ProblemFactory.getInstance().getReferenceSet("ZCAT3_2"));
		Assert.assertEquals(2, ProblemFactory.getInstance().getProblem("ZCAT3_2").getNumberOfObjectives());
		
		Assert.assertNotNull(ProblemFactory.getInstance().getProblem("ZCAT3_3"));
		Assert.assertNull(ProblemFactory.getInstance().getReferenceSet("ZCAT3_3"));
		Assert.assertEquals(3, ProblemFactory.getInstance().getProblem("ZCAT3_3").getNumberOfObjectives());
	}
