...
public class RosenbrockTest extends ProblemTest {
	...
	public void test() {		
		try (AbstractSingleObjectiveProblem problem = new Rosenbrock()) {
			...
			Assert.assertEquals(1210121.0, MockSolution.of(problem).atLowerBounds().evaluate().getObjective(0), 0.0000001);
			Assert.assertEquals(810081.0, MockSolution.of(problem).atUpperBounds().evaluate().getObjective(0), 0.0000001);
		}
	}
