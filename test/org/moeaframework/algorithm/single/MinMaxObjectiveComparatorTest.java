...
public class MinMaxObjectiveComparatorTest {
	...
	public void testCalculateFitness() {
		...
		Assert.assertEquals(calculate(MockSolution.of().withObjectives(-1.0, 0.0), weights), 0.0, TestThresholds.SOLUTION_EPS);
		Assert.assertEquals(calculate(MockSolution.of().withObjectives(0.0, -1.0), weights), 0.0, TestThresholds.SOLUTION_EPS);
		Assert.assertEquals(calculate(MockSolution.of().withObjectives(1.0, 1.0), weights), 1.0, TestThresholds.SOLUTION_EPS);
		Assert.assertEquals(calculate(MockSolution.of().withObjectives(1.0, -1.0), weights), 1.0, TestThresholds.SOLUTION_EPS);
